...
export const ProjectGrid = () => {
  ...
  const getColumnItems = (numColumns: number): PortfolioItem[][] => {
    // Create an array of empty arrays, one for each column, using fill(null) to set each element to null
    const columns: PortfolioItem[][] = Array(numColumns)
      .fill(null)
      .map(() => []);

    // Loop through the items and distribute them into the columns
    portfolioItems.forEach((item: PortfolioItem, index: number) => {
      // Calculate the column index based on the item's index => 0, 1, 2, 0, 1, 2
      const columnIndex: number = index % numColumns;

      // Push the item into the corresponding column
